...
| `CatRoot` copy on a live Windows 11 install, Hyper-V | `\Windows\System32\CatRoot` (1854 catalog files, names up to 136 chars, 28 MB) copied to a second directory on the same 63 GB NTFS volume, one insert per file until the directory record can hold no further mapping pair | 987 created, 987 visible to Windows, 0 orphans, `chkdsk` reports *found no problems*; the remaining inserts are refused with `EFI_UNSUPPORTED` and change nothing. The same test against the previous build reported 964 created, **0** visible, and `chkdsk` recovered 964 orphans out of a garbage child pointer |
